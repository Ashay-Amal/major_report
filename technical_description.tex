...
All datasets use 256$\times$256 resolution images in JPG/PNG format. Domain X uses human faces from Kaggle (3,000+ images each for face-based styles) or landscapes (2,500+ for Van Gogh). Domain Y contains style-specific frames extracted using our pipeline.
...
The generator follows a ResNet-based encoder-decoder architecture with 9 residual blocks for 256$\times$256 images.
...
\begin{tikzpicture}[
    ...
    \node [block, right=1.5cm of input] (conv1) {Conv 3$\times$3 + IN + ReLU};
    \node [block, right=1.5cm of conv1] (conv2) {Conv 3$\times$3 + IN};
    \node [circle, draw, right=1.5cm of conv2] (add) {$+$};
...
The discriminator uses a PatchGAN architecture that classifies 70$\times$70 overlapping patches.
...
\begin{tikzpicture}[
    ...
    \node [block, fill=gray!30] (input) at (0, 0) {Input\\3$\times$256$\times$256};
    \node [block] (c1) at (3, 0) {Conv 64\\LReLU};
    \node [block] (c2) at (6, 0) {Conv 128\\IN + LReLU};
    \node [block] (c3) at (9, 0) {Conv 256\\IN + LReLU};
    \node [block] (c4) at (12, 0) {Conv 512\\IN + LReLU};
    \node [block, fill=gray!30] (output) at (15, 0) {Output\\1$\times$30$\times$30};
...
The adversarial loss ensures that generated images are indistinguishable from real images in the target domain. We use Least Squares GAN (LSGAN) \cite{bib:lsgan} instead of the original cross-entropy loss for more stable training.
...
\textbf{Why LSGAN:}
\begin{itemize}
    \item Provides smoother gradients compared to binary cross-entropy
    \item Penalizes samples that are far from the decision boundary
    \item Reduces vanishing gradient problems during training
    \item Produces higher quality images with fewer artifacts
\end{itemize}
...
\begin{itemize}
    \item \textbf{Image Size (256$\times$256):} Provides a good balance between quality and computational efficiency. Larger sizes increase memory requirements quadratically while providing diminishing returns for style transfer quality.

    \item \textbf{Batch Size (4):} Limited by GPU memory when training with multiple networks (two generators, two discriminators). Smaller batch sizes also help with training stability for GANs.

    \item \textbf{Learning Rate (0.0002):} Standard learning rate for Adam optimizer in GAN training, as recommended in the original CycleGAN paper.

    \item \textbf{$\beta_1$ = 0.5:} Lower momentum than the default 0.9 helps stabilize GAN training by preventing the optimizer from ``overshooting'' in the adversarial dynamics.

    \item \textbf{Linear Decay:} Learning rate is kept constant for the first 100 epochs, then linearly decayed to zero over the remaining 100 epochs. This allows for initial exploration followed by fine-tuning.

    \item \textbf{9 Residual Blocks:} Standard for 256$\times$256 images. 6 blocks are used for 128$\times$128, and 9 for higher resolutions, following the original CycleGAN architecture.
\end{itemize}
...
\textbf{Image History Buffer:} A buffer of 50 previously generated images stabilizes discriminator training by reducing oscillation and providing diverse training signals.

\textbf{Learning Rate Schedule:} Constant rate (0.0002) for epochs 1-100, then linear decay to 0 for epochs 101-200:
\begin{equation}
...
\textbf{Checkpointing:} Models saved every 10 epochs; best model selected based on FID score and visual quality
...
\textbf{Functional Requirements:} Accept JPG/PNG images; provide 4 style options; preprocess to 256$\times$256; transform using Generator G; output within 3 seconds; save as PNG; support batch processing.

\textbf{Non-Functional Requirements:} Minimum 1,000 images/domain; training $<$5 min/epoch; support up to 1024$\times$1024 input; inference $<$3 seconds; model $<$500MB; VRAM $<$4GB; PyTorch 2.0+; CUDA 11.7+.
