...
This chapter gives a detailed look at the AI-based Neural Style Transfer system. It explains the overall architecture, how the dataset was prepared, and the four style transfer models used in the project—One Piece, Disney, Studio Ghibli, and Van Gogh.

\section{System Architecture Overview}

The system follows a modular pipeline that converts user-uploaded content images into artistic versions using CycleGAN-based neural networks. Its architecture includes four different style transfer modes, each trained on carefully curated custom datasets.
...
The system includes four separate style transfer modes:\\
\vspace{-0.9cm}
...
Because CycleGAN works with unpaired datasets from two domains (Domain X: real-world images and Domain Y: stylized images), we created a complete pipeline for generating these datasets.
...
All datasets consist of 256×256 resolution images in JPG or PNG format. For Domain X, we used human face images from Kaggle (over 3,000 images for the face-based styles) and landscape images (more than 2,500 for the Van Gogh model). Domain Y includes style-specific frames that were extracted through our custom pipeline.
